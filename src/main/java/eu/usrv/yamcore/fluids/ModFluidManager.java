...
public class ModFluidManager {
    ...
    public ModSimpleBaseFluid GetModFluid(String pModItemName) {
        if (FluidCollection.containsKey(pModItemName)) return FluidCollection.get(pModItemName);
        else return null;
    }
    ...
    public boolean AddItemToManagedRegistry(ModSimpleBaseFluid pModFluid) {
        if (GetModFluid(pModFluid) == null) {
            ...
            _mLog.debug(
                    String.format("Adding Fluid to managed registry: %s", pModFluid.getFluid().getUnlocalizedName()));
            return true;
        } else {
            _mLog.debug(
                    String.format(
                            "Not adding Fluid to managed registry, since it's already registered! > %s",
                            pModFluid.getFluid().getUnlocalizedName()));
            return false;
        }
    }
            ...
            for (ModSimpleBaseFluid modFluid : FluidCollection.values()) {
                try {
                    ...
                    if (tTargetTab == null) {
                        ...
                        _mLog.warn(
                                String.format(
                                        "CreativeTab name %s requested, but not registered in TabManager. Adding fluid %s to Tab 'Misc'",
                                        modFluid.getCreativeTabName(),
                                        modFluid.getUnlocalizedName()));
                    }
                    ...
                    if (tFluid == null) {
                        _mLog.error(
                                String.format(
                                        "Unable to grab fluid for preregistered modFluid %s",
                                        modFluid.getUnlocalizedName()));
                        continue;
                    }
                    _mLog.debug(
                            String.format("Grabbed fluid: %s Preparing to register...", tFluid.getUnlocalizedName()));
                    ...
                    _mLog.debug(
                            String.format(
                                    "FluidBlock: %s Fluid: %s",
                                    modFluid.getUnlocalizedName(),
                                    tFluid.getUnlocalizedName()));
                    ...
                    if (modFluid.getRegisterBucket()) {
                        ...
                        GameRegistry.registerItem(
                                fluidBucket,
                                _mModID + "_" + fluidBucket.getUnlocalizedName().substring(5));
                        BucketHandler.INSTANCE.buckets.put(modFluid, fluidBucket);
                    }

                    Block tB = GameRegistry
                            .registerBlock(modFluid, _mModID + "_" + modFluid.getUnlocalizedName().substring(5));
                    if (tB == null)
                    ...
                    _mLog.error(
                            String.format("Error while registering fluid %s, skipping", modFluid.getUnlocalizedName()));
                    _mLog.DumpStack(e);
                    continue;
                }
            }
