...
public class ModSimpleBaseFluid extends BlockFluidClassic {
    ...
    /**
     * Add pEffect to the list of potion effects that will stack over time as long as the entity remains in the fluid
     * 
     * @param pEffect
     */
    ...
    /**
     * If set to true, the potion effects will only be applied to Players. If you don't set anything here, ALL entities
     * will get the potion effects
     * 
     * @param pFlag
     */
    ...
    public boolean canDisplace(IBlockAccess world, int x, int y, int z) {
        if (world.getBlock(x, y, z).getMaterial().isLiquid()) return _mCanDisplaceFluids;
        else return super.canDisplace(world, x, y, z);
    }

    @Override
    public boolean displaceIfPossible(World world, int x, int y, int z) {
        if (world.getBlock(x, y, z).getMaterial().isLiquid()) return _mCanDisplaceFluids;
        else return super.displaceIfPossible(world, x, y, z);
    }
    ...
    public void onEntityCollidedWithBlock(World pWorld, int pX, int pY, int pZ, Entity pEntity) {
        ...
        try {
            if (!(pEntity instanceof EntityLivingBase)) return;

            if (!(pEntity instanceof EntityPlayer) && _mEffectsOnlyToPlayers) return;

            EntityLivingBase tElivBase = (EntityLivingBase) pEntity;

            for (PotionEffect tEffect : _mPotionEffects) tElivBase.addPotionEffect(tEffect);
            ...
            if (_mRand.nextInt(10) == 0) {
                for (PotionEffect tStackingEffect : _mStackingPotionEffects) {
                    ...
                    tElivBase.addPotionEffect(
                            new PotionEffect(tStackingEffect.getPotionID(), tDuration, tStackingEffect.getAmplifier()));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            YAMCore.instance.getLogger().error(
                    String.format("Apply potioneffect failed for fluidblock %s", this.getFluid().getUnlocalizedName()));
        }
    }
}
