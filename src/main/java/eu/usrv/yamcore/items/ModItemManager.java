...
public class ModItemManager {
    ...
    public Item GetModItem(String pModItemName) {
        if (ItemCollection.containsKey(pModItemName)) return ItemCollection.get(pModItemName).getConstructedItem();
        else return null;
    }
            ...
            for (ModSimpleBaseItem modItem : ItemCollection.values()) {
                try {
                    ...
                    if (tTargetTab == null) {
                        _mLog.warn(
                                String.format(
                                        "CreativeTab name %s requested, but not registered in TabManager. Adding item %s to Tab 'Misc'",
                                        modItem.getCreativeTabName(),
                                        modItem.getConstructedItem().getUnlocalizedName()));
     ...
     * @return
     */
    public <T extends Item> boolean RegisterNonEnumItem(CreativeTabsManager pTabManager, IExtendedModItem<T> pModItem) { // Failed
                                                                                                                         // to
                                                                                                                         // define
                                                                                                                         // with
                                                                                                                         // "Class
        ...
        try {
            ...
            if (tTargetTab == null) {
                _mLog.warn(
                        String.format(
                                "CreativeTab name %s requested, but not registered in TabManager. Adding item %s to Tab 'Misc'",
                                pModItem.getCreativeTabName(),
                                pModItem.getConstructedItem().getUnlocalizedName()));
