...
public class ModSimpleBaseItem {
    ...
    public ModSimpleBaseItem(IExtendedItemProperties pItemProperties, String pCreativeTabName) {
        ...
        if (pItemProperties.getItemRecipeBehavior() == ItemRecipeBehaviorEnum.NoConsume
                || pItemProperties.getItemRecipeBehavior() == ItemRecipeBehaviorEnum.NoConsumeLeaveInGrid)
    ...
    public void setModIDName(String pModID) {
        ...
        String tTextureName = "";
        if (_mTextureOverride.length() < 1) tTextureName = String.format("%s:item%s", pModID, _mName);
        else tTextureName = String.format("%s:%s", pModID, _mTextureOverride);
