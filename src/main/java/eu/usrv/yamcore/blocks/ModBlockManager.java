...
public class ModBlockManager {
    ...
    public BlockBase GetModBlock(String pModBlockName) {
        if (BlockCollection.containsKey(pModBlockName)) return BlockCollection.get(pModBlockName).getConstructedBlock();
        else return null;
    }
            ...
            for (ModSimpleBaseBlock modBlock : BlockCollection.values()) {
                try {
                    ...
                    if (tTargetTab == null) {
                        _mLog.warn(
                                String.format(
                                        "CreativeTab name %s requested, but not registered in TabManager. Adding Block %s to Tab 'Misc'",
                                        modBlock.getCreativeTabName(),
                                        modBlock.getConstructedBlock().getUnlocalizedName()));
                    ...
                    GameRegistry.registerBlock(
                            modBlock.getConstructedBlock(),
                            modBlock.getConstructedBlock().getUnlocalizedName());
                } catch (Exception e) {
                    _mLog.error(
                            String.format("Error while registering item %s, skipping", modBlock.getUnlocItemName()));
                    _mLog.DumpStack(e);
                    continue;
                }
            }
