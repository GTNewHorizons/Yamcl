...
public class ModSimpleBaseBlock {
	...
	/**
	 * Create a new (still simple) Block by using a helper class to provide additional Information about the block's properties
	...
	public ModSimpleBaseBlock(IExtendedBlockProperties pBlockProperties, String pCreativeTabName)
	{
		...
		if (pBlockProperties.getUnbreakable())
			_mBlockInstance.setBlockUnbreakable();
		else
		{
	...
	public void setModIDName(String pModID)
	{
		...
		if (_mTextureOverride.length() < 1)
			tTextureName = String.format("%s:block%s", pModID, _mName);
		else
			tTextureName = _mTextureOverride;
