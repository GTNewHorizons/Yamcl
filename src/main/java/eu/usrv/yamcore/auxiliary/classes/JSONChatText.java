...
public class JSONChatText {
    ...
    public static JSONChatText simpleMessageWCommand(String pMessage, String pCommand, boolean pSuggestOnly) {
        JSONChatText tJ = new JSONChatText();
        tJ.Message = pMessage;
        if (pSuggestOnly) tJ.ClickEvent = JSONClickEvent.suggestCommand(pCommand);
        else tJ.ClickEvent = JSONClickEvent.runCommand(pCommand);
        return tJ;
    }
    ...
    /**
     * Display text from the localized language. e.g. item.stone for the localized name for stone. This string may also
     * contain one or multiple placeholder (%s). For each placeholder, you must create an entry in TranslateArray. The
     * first %s is TranslateArray[0], the second [1],...
     */
    ...
    public String getConstructed() {
        ...
        if (Message != "") tResultObject.addProperty("text", Message);
        if (Translate != "") tResultObject.addProperty("translate", Translate);

        if (TranslateArray != null) tResultObject.add("with", new Gson().toJsonTree(TranslateArray));

        if (Bold != false) tResultObject.addProperty("bold", true);
        if (Italic != false) tResultObject.addProperty("italic", true);
        if (Underlined != false) tResultObject.addProperty("underlined", true);
        if (Strikethrough != false) tResultObject.addProperty("strikethrough", true);
        if (Obfuscated != false) tResultObject.addProperty("obfuscated", true);
        if (Color != null) tResultObject.addProperty("color", Color.getFriendlyName());

        if (HoverEvent != null) tResultObject.add("hoverEvent", HoverEvent.getJson());

        if (ClickEvent != null) tResultObject.add("clickEvent", ClickEvent.getJson());
