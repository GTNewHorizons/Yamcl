...
public class Placeable<T> implements Supplier<T> {
     ...
     * @throws NullPointerException if mod object is present and {@code consumer} is null
     */
    public void ifLoaded(Consumer<? super T> consumer) {
        if (isLoaded()) consumer.accept(get());
    }

    /**
     * If a mod object is present, apply the provided mapping function to it, and if the result is non-null, return an
     ...
     * @return an {@code Optional} describing the result of applying a mapping function to the mod object of this
     *         {@code Promised}, if a mod object is present, otherwise an empty {@code Optional}
     ...
     * @param mapper a mapping function to apply to the mod object, if present the mapping function
    ...
    /**
     * If a mod object is present, lazily apply the provided mapping function to it, returning a supplier for the
     * transformed result. If this object is empty, or the mapping function returns {@code null}, the supplier will
     * return {@code null}.
     *
     * @param <U>    The type of the result of the mapping function
     * @param mapper A mapping function to apply to the mod object, if present
     * @return A {@code Supplier} lazily providing the result of applying a mapping function to the mod object of this
     *         {@code Promised}, if a mod object is present, otherwise a supplier returning {@code null}
     ...
     *
     * @param other the mod object to be returned if there is no mod object present, may be null
    ...
    /**
     * Return the mod object if present, otherwise invoke {@code other} and return the result of that invocation.
     *
     * @param other a {@code Supplier} whose result is returned if no mod object is present
     * @return the mod object if present otherwise the result of {@code other.get()}
     * @throws NullPointerException if mod object is not present and {@code other} is null
     */
     ...
     * @throws NullPointerException if no mod object is present and {@code exceptionSupplier} is null
