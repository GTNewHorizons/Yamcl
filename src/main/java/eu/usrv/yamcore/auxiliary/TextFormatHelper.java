...
public class TextFormatHelper {
    ...
    public static String DecodeStringCodes(String pSource, String pToken) {
        return pSource.replace(addToken(pToken, "0"), FontCodes.BLACK)
                ...
                .replace(addToken(pToken, "3"), FontCodes.DARK_AQUA).replace(addToken(pToken, "4"), FontCodes.DARK_RED)
                .replace(addToken(pToken, "5"), FontCodes.DARK_PURPLE).replace(addToken(pToken, "6"), FontCodes.GOLD)
                .replace(addToken(pToken, "7"), FontCodes.GRAY).replace(addToken(pToken, "8"), FontCodes.DARK_GREY)
                .replace(addToken(pToken, "9"), FontCodes.BLUE).replace(addToken(pToken, "a"), FontCodes.GREEN)
                .replace(addToken(pToken, "b"), FontCodes.AQUA).replace(addToken(pToken, "c"), FontCodes.RED)
                .replace(addToken(pToken, "d"), FontCodes.LIGHT_PURPLE).replace(addToken(pToken, "e"), FontCodes.YELLOW)
                .replace(addToken(pToken, "f"), FontCodes.WHITE).replace(addToken(pToken, "k"), FontCodes.OBFUSCATED)
                .replace(addToken(pToken, "l"), FontCodes.BOLD).replace(addToken(pToken, "m"), FontCodes.STRIKETHROUGH)
                .replace(addToken(pToken, "n"), FontCodes.UNDERLINE).replace(addToken(pToken, "o"), FontCodes.ITALICS)
                .replace(addToken(pToken, "r"), FontCodes.RESET);
    }

    /**
     * Decodes any font codes into something useable by the FontRenderer. The tokens (0-9, a-o are minecraft codes. Use
     * token r to reset any format)
     * 
