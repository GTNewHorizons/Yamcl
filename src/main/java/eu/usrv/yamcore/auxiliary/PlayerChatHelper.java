...
public class PlayerChatHelper {
    ...
    /**
     * Just plain white text without any format or color Just pass your args. No need for String.format()
     * 
     * @param pCommandSender
     * @param pMessage
     * @param pArgs
    ...
    /**
     * Meant for notifications that are being send to an admin/op Color will be GREEN Just pass your args. No need for
     * String.format()
     * 
     * @param pCommandSender
     * @param pMessage
    ...
    /**
     * Meant for notifications that are being send to an admin/op Color will be RED Just pass your args. No need for
     * String.format()
     * 
     * @param pCommandSender
     * @param pMessage
    ...
    /**
     * Meant for notifications that are being send to an admin/op Color will be YELLOW Just pass your args. No need for
     * String.format()
     * 
     * @param pCommandSender
     * @param pMessage
    ...
    /**
     * Meant for ingame notifications that are being send to a player, not an admin/op Color will be DARK_GREEN Just
     * pass your args. No need for String.format()
     * 
     * @param pPlayer
     * @param pMessage
     */
    ...
    /**
     * Meant for ingame notifications that are being send to a player, not an admin/op Color will be AQUA Just pass your
     * args. No need for String.format()
     * 
     * @param pPlayer
     * @param pMessage
     */
    ...
    /**
     * Meant for ingame notifications that are being send to a player, not an admin/op Color will be DARK_PURPLE Just
     * pass your args. No need for String.format()
     * 
     * @param pPlayer
     * @param pMessage
    ...
    /**
     * Send any combination of links, colors, Tooltips like you would with String.format(). Use the following tokens to
     * fill in the json chat objects: "This is a {0} sample message that will {1} display elements based on {0} their
     * index value {2}" {0} refers to the first given object, {1} to the second, etc. If you want to use { and } in your
     * text, escape them like any other special char: \{
     * 
     ...
     * @throws Exception
     */
    public static void SendJsonFormatted(EntityPlayer pPlayer, String pMessage, JSONChatText... pChatObjects)
            throws Exception {
        ...
        for (int i = 0; i < tMessageArray.length; i++) {
            ...
            if (tokenStart > -1 && tokenEnd > -1) {
                ...
                if (!IntHelper.tryParse(tArgNr)) throw new IllegalArgumentException(
                        String.format("Token-Element at pos %d is not an integer (%s)", tokenStart + 1, tArgNr));

                int tArg = Integer.parseInt(tArgNr);

                if (pChatObjects.length < tArg) throw new IndexOutOfBoundsException(
                        String.format("Token-ID %d requested, but only given %d !", tArg, pChatObjects.length));
    ...
    private static JSONChatText extractSubStringToJSON(String pMessage, int pStart, int pEnd) {
        ...
        // Create an empty json body with the text and add it to the collection
        // YAMCore.instance.getLogger().info( String.format("Extracted [%s] from message. Start: %d End: %d",
        // tMessagePart,
        return JSONChatText.simpleMessage(tMessagePart);
    }

    /**
     * Send variable number of JSON Objects to a player. Note: It is recommended to use SendJsonFormatted() if you want
     * to format a text This function will just glue all elements together!
     * 
     * @param pPlayer
     * @param pChatObjects
     */
        ...
        if (tMCServer != null) {
            ...
            if (pChatObjects.length > 1) {
                ...
                for (JSONChatText pChatObject : pChatObjects) {
                    ...
                    else tConstructedJSON += ", ";

                    tConstructedJSON += pChatObject.getConstructed();
                }
                tConstructedJSON += "]";
            } else tConstructedJSON = pChatObjects[0].getConstructed();
